...
const premium = JSON.parse(fs.readFileSync('./𝙲𝚘𝚗𝚝𝚛𝚘𝚕𝚕𝚎𝚛/system/premium.json'))
...
const addPremiumUser = (userId, expired, _dir) => {
	...
	fs.writeFileSync("./𝙲𝚘𝚗𝚝𝚛𝚘𝚕𝚕𝚎𝚛/system/premium.json", JSON.stringify(_dir));
};
...
const expiredPremiumCheck = (danadyaksa, msg, _dir) => {
	setInterval(() => {
		let position = null;
		Object.keys(_dir).forEach((i) => {
		...
		if (position !== null) {
			...
			fs.writeFileSync("./𝙲𝚘𝚗𝚝𝚛𝚘𝚕𝚕𝚎𝚛/system/premium.json", JSON.stringify(_dir));
			idny ? danadyaksa.sendMessage(idny, { text: "Your premium has run out, please buy again." }) : "";
			idny = false;
		}
	}, 1000);
};
